...
class GPT2Model(GPTPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    final_embeds = self.embed_dropout(inputs_embeds + pos_embeds)
  ...
  def encode(self, hidden_states, attention_mask):
    ...
    # Repeat the forward pass recurrently for r steps
    for r in range(self.config.recurrency - 1):
      with torch.no_grad():
  ...
  @classmethod
  def from_pretrained(cls, model='gpt2', d=768, l=12, num_heads=12, causal_attention=True, recurrency=1):
    ...
    our_model = GPT2Model(GPT2Config(hidden_size=d, num_hidden_layers=l,num_attention_heads=num_heads,
                                     intermediate_size=d*3, causal_attention=causal_attention, recurrency=recurrency)).eval()
